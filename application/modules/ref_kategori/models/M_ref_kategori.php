...
class M_ref_kategori extends CI_Model {
    ...
    public function getDataById($id){
        ...
        $sql = $this->query()." WHERE id_category = '$id'";
        $query = $this->db->query($sql);
    ...
    public function delete($where, $permanent = FALSE){
        if ($permanent)
            ...
            $result= $this->db->update($this->table_name, ['c_is_deleted' => 1], $where);
        return $result;
    }

    public function restore($where){
        $result= $this->db->update($this->table_name, ['c_is_deleted' => 0], $where);
        return $result;
    }
}

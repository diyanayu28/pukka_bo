...
class M_service extends CI_Model {
    ...
    protected $tag_service = 'tag_service';

    private function query(){
        $query  =   "SELECT * FROM(
                    SELECT
                        s.*, category.*, GROUP_CONCAT(tag_name)AS tag_name
                    FROM service s
                    LEFT JOIN tag_service ON s.id_service = tag_service.fk_id_service
                    LEFT JOIN tag ON tag_service.fk_id_tag = tag.id_tag
                    LEFT JOIN category ON s.fk_id_category = category.id_category
                    GROUP BY id_service)
                    AS temp_table";
        return $query;
    }
    ...
    public function getDataById($id){
        $sql = $this->query()." WHERE id_service = '$id'";
        $query = $this->db->query($sql);
        return $query->row_array();  
    }

    public function getTagById($serviceId){
        $data = $this->db->query("SELECT `fk_id_tag`, `fk_id_service`, `tag_name`, `id_tag`, `service_name`, `id_service`
            FROM `tag_service` AS a
            LEFT JOIN `service` AS b ON a.`fk_id_service` = b.`id_service`
            LEFT JOIN `tag` AS c ON a.`fk_id_tag` = c.`id_tag`
            WHERE
            a.`fk_id_service` = '{$serviceId}'")->result();
        return $data;
    }
    ...
    public function getOptionKategori(){
        $sql = "SELECT
                ...
                FROM category WHERE c_is_deleted = 0 AND category_group = 'service'";
    ...
    public function delete_tag_service($where){
            $result= $this->db->where($where)->delete($this->tag_service);
        return $result;
    }
